...
export default function App() {
  ...
  const changeQty = (id: string, delta: number) => mutate(l => ({ ...l, groups: l.groups.map(group => ({ ...group, sections: group.sections.map(section => ({ ...section, items: section.items.map(item => item.id === id ? { ...item, qty: Math.max(.001, Math.round((item.qty + delta) * 1000) / 1000) } : item) })) })) }));
...
function ShoppingItemRow({ item, selected, onSelect, onStatus, onQty, onDelete }: { item: Item; selected: boolean; onSelect: (checked: boolean) => void; onStatus: () => void; onQty: (delta: number) => void; onDelete: () => void }) {
